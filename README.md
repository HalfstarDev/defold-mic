...
| `max_seconds`  | number | Maximum number of seconds to buffer. Once reached, incoming audio is discarded. `0` or omitted means unlimited. |
...
#### mic.stop()
Stop audio capture and return the recorded audio as a WAV byte string. Returns `nil` if nothing was recorded.
...
#### mic.get_devices()
Enumerate all available capture devices. Returns a list of device info tables, each with keys `index` and `name`.
...
For using the microphone on Android devices, you have to first get the permission `android.permission.RECORD_AUDIO` from the user.